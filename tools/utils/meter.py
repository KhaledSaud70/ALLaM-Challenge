...
class MeterRegistry:
    ...
    @classmethod
    def register(cls, meter_class):
        cls._meters[meter_class.__name__.lower()] = meter_class
        return meter_class
...
class Bahr:
    ...
    def remove_disallowed_tafeelas_in_hashw(
        ...
        for tafeela in tafeela_forms:
            assert (
                tafeela.applied_ella_zehaf_class is None or tafeela.applied_ella_zehaf_class in tafeela.allowed_zehafs
            ), f"zehaf {tafeela.applied_ella_zehaf_class} is not allowed for {tafeela}"
            if (
                tafeela.applied_ella_zehaf_class
                not in self.disallowed_zehafs_for_hashw[shatr_index][tafeela_hashw_index]
            ):
    ...
    def _one_shatr_combinations(self):
        assert isinstance(self.arod_dharbs_map, set), "if only_one_shatr is true, arood_dharbs_map should be a set"
        return list(
            itertools.product(
                *self.get_shatr_hashw_combinations(),
                [arood_class(self.last_tafeela).modified_tafeela for arood_class in self.arod_dharbs_map],
            )
        )
            ...
            first_shatr_combinations = list(
                itertools.product(
                    *self.get_shatr_hashw_combinations(shatr_index=0),
                    ...
                    + [  # for tasree patterns
                        dharb_class(self.last_tafeela).modified_tafeela for dharb_class in dharb_classes
                    ],
                )
            )
    ...
    def all_baits_combinations_patterns(self):
        ...
        for combination in self.bait_combinations:
            ...
            if isinstance(combination[0], Tafeela):
                shatr = combination
                pattern += "".join("".join(map(str, tafeela.pattern)) for tafeela in shatr)
            else:
                first_shatr, second_shatr = combination
                pattern += "".join("".join(map(str, tafeela.pattern)) for tafeela in first_shatr)
                pattern += "".join("".join(map(str, tafeela.pattern)) for tafeela in second_shatr)
    ...
    def get_all_shatr_combinations(self, as_str_list=True):
        ...
        combinations = list(dict.fromkeys(combinations))  # remove and maintain the order
        if as_str_list:
            return [" ".join((str(tafeela) for tafeela in shatr)) for shatr in combinations]
        return combinations
...
class Mudhare(Bahr):
    ...
    def bait_combinations(self):
        ...
        zehafed_combinations = list(
            filter(
                lambda combination: combination[0][0].applied_ella_zehaf_class is not None
