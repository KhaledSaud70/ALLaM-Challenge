...
class BaseDoubledZehaf(BaseEllahZehaf):
    ...
    def modify_tafeela(self):
        ...
        assert all(
            issubclass(zehaf, (BaseSingleHazfZehaf, BaseSingleTaskeenZehaf)) for zehaf in self.zehafs
        ), "zehafs should be derived either from BaseSingleHadhfZehaf or BaseSingleTaskeenZehaf"
        hazf_zehafs = filter(lambda zehaf: issubclass(zehaf, BaseSingleHazfZehaf), self.zehafs)
...
class Tatheel(BaseEllahZehaf):
    ...
    def modify_tafeela(self):
        ...
        self.tafeela.add_to_pattern(index=len(self.tafeela.pattern) - 1, number=0, char_mask="ا")
